...
FILE_SHOP_RESULTS = "tests/homeworks/Homework_6/Homework_6_1/shop_results.txt"
FILE_SHOP_BALANCE = "tests/homeworks/Homework_6/Homework_6_1/shop_balance.txt"
FILE_SHOP_LOGS = "tests/homeworks/Homework_6/Homework_6_1/shop_logs.txt"
...
def test_shop_results():
    storage = create_tree_map()
    result_list = file_scrolling(storage, FILE_SHOP_LOGS)
    with open(FILE_SHOP_RESULTS, "r") as file:
        for i in range(len(result_list)):
            assert result_list[i] == file.readline()


def test_shop_balance():
    storage = create_tree_map()
    file_scrolling(storage, FILE_SHOP_LOGS)
    with open(FILE_SHOP_BALANCE, "r") as file:
        for size, count in traverse(storage, "inorder"):
            assert f"{size} {count}\n" == file.readline()
