...
def start():
    while True:
        ...
        if all([isfile(file_output), isfile(file_input), file_input[-3:] == "txt", file_output[-3:] == "txt", file_input != file_output]):
...
def file_scrolling(file_name_input, file_name_output):
    with open(file_name_input, "r") as file_input, open(file_name_output, "w") as file_output:
        m = int(file_input.readline())
        dna_line = file_input.readline()
        n = int(file_input.readline())
        for i in range(n):
            command, arg1, arg2 = ''.join([i for i in str(file_input.readline()) if i not in "]\n"]).split("[")
            if command == "DELETE": dna_line = delete_fragment(dna_line, arg1, arg2, file_output)
            elif command == "INSERT": dna_line = insert_fragment(dna_line, arg1, arg2, file_output)
            elif command == "REPLACE": dna_line = replace_fragment(dna_line, arg1, arg2, file_output)
            else: file_output.write(dna_line)
