...
def start():
    while True:
        ...
        if isfile(file_output) and isfile(file_input) and file_input[-3:] == "txt" and file_output[-3:] == "csv":
...
def find_dict(file_input):
    ...
    with open(file_input, encoding="utf-8", mode="r") as file:
        for line in file:
            for word in line.translate(str.maketrans('', '', punctuation)).lower().split():
                if word == "-": continue
