...
def main():
    ...
    if is_correct_input(user_input):
        print(
            f"Число Фибоначчи под номером {user_input}: {get_fibonacci_num(int(user_input))}"
        )
    else:
        print(
            "Вы не корректно ввели число! Ваше число должно быть целым, положительным, меньше 91"
        )
