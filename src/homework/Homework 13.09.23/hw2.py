...
def find_vectors():
    vector_1_input = input(
        "Введите первый вектор через пробел, без запятых (Например: x1 y2 z3): "
    )
    vector_2_input = input(
        "Введите второй вектор через пробел, без запятых (Например: x1 y2 z3): "
    )
...
def angle_vectors(vector_1, vector_2):
    if len(vector_1) == len(vector_2):
        if len(vector_1) == 0:
            ...
            vectors_len = (
                sum(x**2 for x in vector_1) ** 0.5
                * sum(y**2 for y in vector_2) ** 0.5
            )
            return (
                acos(
                    sum(vector_1[i] * vector_2[i] for i in range(len(vector_1)))
                    / vectors_len
                )
                * 180
                / pi
...
def line_matrix_create(line, count_columns):
    line_matrix = input(
        f"Введите содержания {line + 1} строки через пробел, без запятых (Например: a1 a2 a3): "
    )
    if len(line_matrix.split()) == count_columns and all(
        a.isdigit() for a in line_matrix.split()
    ):
...
def matrix_create():
    ...
    if count_lines.isdigit() and count_columns.isdigit():
        return [
            line_matrix_create(line, int(count_columns))
            for line in range(int(count_lines))
        ]
    else:
...
def matrix_multiplication(matrix_1, matrix_2):
    if len(matrix_1) == len(matrix_2[0]):
        ...
        return [
            [scalar_product(line_1, line_2) for line_2 in matrix_2]
...
def start_vector():
    ...
    if vector_func == "1":
        print(scalar_product(vector_1, vector_2))
    elif vector_func == "2":
        print(", ".join(map(str, len_vectors(vector_1, vector_2))))
    elif vector_func == "3":
        print(angle_vectors(vector_1, vector_2))
    else:
...
def start():
    while True:
        vector_or_matrix = str(
            input("Выберите цифру от 1 до 2: \n(1) Вектора \n(2) Матрица \n")
        )
        if vector_or_matrix == "1":
            start_vector()
        elif vector_or_matrix == "2":
            start_matrix()
